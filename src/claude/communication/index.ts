...
export async function processNextMessage(): Promise<void> {
    ...
    if (messageQueue.length === 0) {
        ...
        updateWebviewContent();
        updateSessionState();
        showInfo('All messages processed. Claude session remains active. Add messages to continue.');
    ...
    if (!message) {
        ...
        updateWebviewContent();
        updateSessionState();
        showInfo('No pending messages to process. Processing remains active.');
        return;
    }

    if (!claudeProcess) {
        debugLog('❌ Claude process not available');
        showWarning('Claude session not started. Please start Claude session first.');
    ...
    if (!sessionReady) {
        ...
        setTimeout(() => {
            ...
            if (!sessionReady) {
                ...
                showWarning('Claude session failed to become ready. Please restart the session.');
                setProcessingQueue(false);
                setIsRunning(false);
            }
        }, 30000);
...
export async function startProcessingQueue(skipPermissions: boolean = true): Promise<void> {
    ...
    if (!claudeProcess) {
        showInfo('Starting Claude session...');
        ...
        if (messageQueue.length > 0) {
            ...
            setTimeout(() => {
                clearInterval(checkReadyInterval);
                if (!sessionReady) {
                    showError('Claude session failed to start. Please check your Claude CLI installation.');
                }
            }, 30000);
        }
    ...
    if (messageQueue.length === 0) {
        showInfo('Claude session is ready. Add messages to start processing.');
        return;
    }
