...
export function enforceQueueSizeLimit(): void {
    ...
    if (removedCount > 0) {
        ...
        showInfo(`Queue cleanup: Removed ${removedCount} old completed messages`);
        updateWebviewContent();
    } else {
        // If no completed messages to remove, show warning but don't remove pending messages
        showWarning(`Queue is full (${messageQueue.length} messages). Consider clearing completed messages.`);
    }
}
