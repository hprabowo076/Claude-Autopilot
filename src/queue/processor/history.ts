...
import { standaloneState } from '../../core/state/standalone-state';
...
export function saveWorkspaceHistory(): void {
    ...
    try {
        ...
        const existingHistory = standaloneState.globalState.get<HistoryRun[]>(storageKey, []);
        ...
        standaloneState.globalState.update(storageKey, recentHistory);
...
export function loadWorkspaceHistory(): void {
    ...
    const storageKey = getHistoryStorageKey();
    const history = standaloneState.globalState.get<HistoryRun[]>(storageKey, []);
...
export function filterHistory(filter: string): void {
    ...
    const storageKey = getHistoryStorageKey();
    const allHistory = standaloneState.globalState.get<HistoryRun[]>(storageKey, []);
...
export function savePendingQueue(): void {
    ...
    standaloneState.globalState.update(storageKey, pendingMessages);
}
...
export function loadPendingQueue(): void {
    ...
    const storageKey = getPendingQueueStorageKey();
    const pendingMessages = standaloneState.globalState.get<MessageItem[]>(storageKey, []);
    
    if (pendingMessages.length > 0) {
        messageQueue.splice(0, messageQueue.length, ...pendingMessages);
        showInfo(`Restored ${pendingMessages.length} pending messages from previous session`);
    }
}
...
export function clearPendingQueue(): void {
    ...
    const storageKey = getPendingQueueStorageKey();
    standaloneState.globalState.update(storageKey, []);
}
...
export function deleteHistoryRun(runId: string): void {
    ...
    try {
        ...
        standaloneState.globalState.update(storageKey, filteredHistory);
