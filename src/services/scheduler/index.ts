...
export function startScheduledSession(callback: () => void): void {
    ...
    try {
        ...
        scheduledTimer = setTimeout(() => {
            ...
            showInfo('🚀 Starting scheduled Claude session...');
            callback();
        ...
        showInfo(`Claude session scheduled to start at ${timeString}`);
        
    } catch (error) {
        debugLog(`❌ Failed to schedule session start: ${error}`);
        showError(`Invalid scheduled start time format: ${config.session.scheduledStartTime}`);
    }
}
